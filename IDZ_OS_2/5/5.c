...
void init_semaphores() {
  sem_init(&hotel->sem_single, 1, SINGLE_ROOMS);
  sem_init(&hotel->sem_double, 1, DOUBLE_ROOMS);
...
void destroy_semaphores() {
  sem_destroy(&hotel->sem_single);
  sem_destroy(&hotel->sem_double);
...
void customer(bool is_male) {
  while (!should_exit) {
    ...
    if (is_male) {
      if (sem_trywait(&hotel->sem_single) == 0) {
        ...
        sem_post(&hotel->sem_mutex);
        sem_post(&hotel->sem_single);
      } else if (sem_trywait(&hotel->sem_double) == 0) {
        sem_wait(&hotel->sem_mutex);
        hotel->double_rooms--;
        ...
        sem_post(&hotel->sem_mutex);
        sem_post(&hotel->sem_double);
      } else {
        printf("Мужчина уходит, нет свободных номеров.\n");
      }
    } else {
      if (sem_trywait(&hotel->sem_double) == 0) {
        sem_wait(&hotel->sem_mutex);
        hotel->double_rooms--;
        hotel->double_occupants++;
